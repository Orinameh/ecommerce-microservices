...
describe('CustomerController', () => {
  ...
  test('getCustomerById returns 404 when not found', async () => {
    const ctrl = createController({ getCustomerById: mock(() => Promise.reject(new Error('Customer not found'))) });
    const { req, res } = createReqRes();
  ...
  test('createCustomer returns 409 on duplicate email', async () => {
    const ctrl = createController({ createCustomer: mock(() => Promise.reject(new Error('Customer with this email already exists'))) });
    const { req, res } = createReqRes();
  ...
  test('deleteCustomer returns 404 when not found', async () => {
    const ctrl = createController({ deleteCustomer: mock(() => Promise.reject(new Error('Customer not found'))) });
    const { req, res } = createReqRes();
    req.params = { id: 'nonexistent' };
